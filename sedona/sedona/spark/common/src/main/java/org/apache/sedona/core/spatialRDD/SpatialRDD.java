...
public class SpatialRDD<T extends Geometry> implements Serializable {
   ...
   */
  public void calc_partitioner(GridType gridType, int numPartitions, StandardQuadTree precomputedTree) throws Exception {

          List<Envelope> samples = null;
      ...
      case QUADTREE:
        {
                        ...
                        {
                                partitioner = new QuadTreePartitioner(precomputedTree);
                        }
  ...
  public Object userDefinedSpatialPartitioning(GridType gridType, StandardQuadTree precomputedTree) throws Exception
  {
	  
	  int numPartitions = this.rawSpatialRDD.rdd().partitions().length;
	 
      if(numPartitions < 12) {numPartitions = 12;} 
      calc_partitioner(gridType, numPartitions, precomputedTree);

      QuadTreePartitioner qtpartitioner = (QuadTreePartitioner) partitioner;

      this.spatialPartitionedRDD = partition(partitioner);

      return qtpartitioner.getQuadTree();
